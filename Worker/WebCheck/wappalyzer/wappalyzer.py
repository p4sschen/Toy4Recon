...
class Wappalyzer(object):
    ...
    def latest(cls, apps_file=None):
        ...
        if apps_file:
            ...
            # fp = open("data/apps.json", encoding='utf-8')
            obj = json.load(fp)
...
def worker_entry(websites):
    ...
    for website in websites:
        ...
        try:
            webpage = WebPage.new_from_url(entireUrl)
        except Exception as E:
            logger.warning(E)
            continue
        techs = wappalyzer.analyze_with_categories(webpage)
        result.append({'website': website, 'techs': techs})
    return result

# worker_entry('http://jqcrm.faw.com.cn/')
